...
- **Dashboard** - shows your most overdue problem up front, then your full review queue sorted by how overdue each problem is. You can search, mark problems done with a 5-second undo, and see today's stats at a glance.
- **Problem Library** - browse all your problems with filters for category, difficulty, and status (New / Attempted / Mastered). Grouped by NeetCode category with collapsible sections.
- **Problem Detail** - edit metadata, write study notes (auto-saved), add a code snippet with a language selector and syntax highlighting, log time/space complexity, and mark as done to advance the schedule.
- **Review Log** - full account-wide history of every review you've ever logged, with filters and links back to each problem.
...
After each review, the next due date is scheduled at: +1, +3, +7, +14, then +30 days (capped there). If you let a problem go more than twice its scheduled interval, the streak resets to zero. Editing or deleting a past review rebuilds the entire chain so your intervals stay consistent. The algorithm lives in [`packages/shared/src/scheduler.ts`](packages/shared/src/scheduler.ts) and is shared between the API and frontend.
